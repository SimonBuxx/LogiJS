...
function deleteInvalidDiodes() {
    for (var j = diodes.length - 1; j >= 0; j--) {
...
function deleteInvalidConpoints() {
    for (var j = conpoints.length - 1; j >= 0; j--) {
...
function getGroup(seg) {
    for (var i = 0; i < groups.length; i++) {
        for (var j = 0; j < groups[i].segments.length; j++) {
...
function doConpoints() {
    for (var i = 0; i < segments.length; i++) {
        ...
        var wp1 = wirePoints(segments[i].startX, segments[i].startY, -1);
        var wp2 = wirePoints(segments[i].endX, segments[i].endY, -1);
